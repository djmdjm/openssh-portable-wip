...
public_fuzz(struct sshkey *k)
{
	...
	/* MLDSA65_ED25519 keys are large and slow to fuzz */
	if (test_is_fast() || sshkey_type_plain(k->type) == KEY_MLDSA65_ED25519)
...
sshkey_fuzz_tests(void)
{
	...
	/* MLDSA65-ED25519 are large and slow to fuzz */
	TEST_START("fuzz MLDSA65-ED25519 private");
	...
	TEST_START("fuzz MLDSA65-ED25519 public");
	buf = load_file("mldsa65_ed25519_1");
	ASSERT_INT_EQ(sshkey_parse_private_fileblob(buf, "", &k1, NULL), 0);
	sshbuf_free(buf);
	public_fuzz(k1);
	...
	TEST_START("fuzz MLDSA65-ED25519 cert");
	ASSERT_INT_EQ(sshkey_load_cert(
	    test_data_file("mldsa65_ed25519_1"), &k1), 0);
	public_fuzz(k1);
	sshkey_free(k1);
	TEST_DONE();
	...
	TEST_START("fuzz MLDSA65-ED25519 sig");
	buf = load_file("mldsa65_ed25519_1");
	ASSERT_INT_EQ(sshkey_parse_private_fileblob(buf, "", &k1, NULL), 0);
	sshbuf_free(buf);
	sig_fuzz(k1, NULL);
