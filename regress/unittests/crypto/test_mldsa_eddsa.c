...
{
	struct sshbuf *tmp, *ret;
	char *k, *b64;
	size_t off, end_off;
	u_char c;

	tmp = sshbuf_from(sshbuf_ptr(content), sshbuf_len(content));
	...
	ASSERT_PTR_NE(b64 = malloc(end_off + 1), NULL);
	memcpy(b64, sshbuf_ptr(tmp), end_off);
	b64[end_off] = '\0';
    ...
    const uint8_t *pk, const uint8_t *s)
{
	...
	ASSERT_INT_EQ(crypto_sign_mldsa65_ed25519_verify(s, m, m_len,
	    ctx, ctx_len, pk), 0);
	fuzz = fuzz_begin(fuzzers, s, MLDSA65_ED25519_SIG_SZ);
	TEST_ONERROR(onerror, fuzz);
	for(; !fuzz_done(fuzz); fuzz_next(fuzz)) {
		/* Ensure 1-bit difference at least */
		if (fuzz_matches_original(fuzz))
			continue;
		ASSERT_INT_NE(crypto_sign_mldsa65_ed25519_verify(fuzz_ptr(fuzz),
...
mldsa_eddsa_tests(void)
{
	...
	TEST_START("MLDSA65-Ed25519-SHA512 load test vectors");
	...
	ASSERT_INT_EQ(sshbuf_len(b_pk), MLDSA65_ED25519_PK_SZ);
	ASSERT_INT_EQ(sshbuf_len(b_sk), MLDSA65_ED25519_SK_SZ);
	ASSERT_INT_EQ(sshbuf_len(b_s), MLDSA65_ED25519_SIG_SZ);
	ASSERT_INT_EQ(sshbuf_len(b_sWithContext), MLDSA65_ED25519_SIG_SZ);
	TEST_DONE();
	...
	ASSERT_INT_EQ(crypto_sign_mldsa65_ed25519_keygen(pk, sk), 0);
	ASSERT_INT_EQ(crypto_sign_mldsa65_ed25519_sign(sig,
	    tvec_m, tvec_m_len, tvec_ctx, tvec_ctx_len, sk), 0);
	ASSERT_INT_EQ(crypto_sign_mldsa65_ed25519_verify(sig,
	...
	ASSERT_INT_EQ(crypto_sign_mldsa65_ed25519_keygen_seeded(pk_expanded,
	    sk_expanded, tvec_sk, tvec_sk + 32), 0);
	ASSERT_MEM_EQ(pk_expanded, tvec_pk, MLDSA65_ED25519_PK_SZ);
	/* sk_expanded should also match tvec_sk */
	ASSERT_MEM_EQ(sk_expanded, tvec_sk, MLDSA65_ED25519_SK_SZ);
	TEST_DONE();

	TEST_START("MLDSA65-Ed25519-SHA512 raw KAT verify (no context)");
	ASSERT_INT_EQ(crypto_sign_mldsa65_ed25519_verify(tvec_s, tvec_m,
	    tvec_m_len, NULL, 0, tvec_pk), 0);
	TEST_DONE();

	TEST_START("MLDSA65-Ed25519-SHA512 raw KAT verify (with context)");
	ASSERT_INT_EQ(crypto_sign_mldsa65_ed25519_verify(tvec_sWithContext,
	    tvec_m, tvec_m_len, tvec_ctx, tvec_ctx_len, tvec_pk), 0);
	TEST_DONE();

	TEST_START("MLDSA65-Ed25519-SHA512 raw round-trip (no context)");
	ASSERT_INT_EQ(crypto_sign_mldsa65_ed25519_sign(sig, tvec_m, tvec_m_len,
	    NULL, 0, tvec_sk), 0);
	ASSERT_INT_EQ(crypto_sign_mldsa65_ed25519_verify(sig,
	    tvec_m, tvec_m_len, NULL, 0, tvec_pk), 0);
	TEST_DONE();

	TEST_START("MLDSA65-Ed25519-SHA512 raw round-trip (with context)");
	ASSERT_INT_EQ(crypto_sign_mldsa65_ed25519_sign(sig,
	...
	TEST_START("MLDSA65-Ed25519-SHA512 fuzz raw verify (with context)");
	ASSERT_INT_EQ(crypto_sign_mldsa65_ed25519_verify(tvec_sWithContext,
	    tvec_m, tvec_m_len, tvec_ctx, tvec_ctx_len, tvec_pk), 0);
