...
rm -f rsa ecdsa ed25519 ecdsa_sk ed25519_sk mldsa65-ed25519
rm -f rsa.sig ecdsa.sig ed25519.sig ecdsa_sk.sig ed25519_sk.sig
rm -f mldsa65-ed25519.sig
...
ssh-keygen -t mldsa65-ed25519 -C "MLDSA65-ED25519 test key" \
    -N "" -f mldsa65-ed25519
...
ssh-keygen -Y sign -f mldsa65-ed25519 -n $NAMESPACE - \
	< signed-data > mldsa65-ed25519.sig
